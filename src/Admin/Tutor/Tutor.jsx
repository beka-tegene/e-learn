...
import AddTeacherForm from "./AddTutorForm";
...
const Tutor = () => {
  ...
  const response = async (page, limit) => {
    try {
      ...
      const { data } = await axios.get(
        `${back_base_url}api/v1/users/getallusers?&role=tutorinstructor&page=${page}&limit=${limit}`
      );
      console.log(data);
      setTutor(data.users);
  ...
  const handleRowClick = (row) => {
    navigate(`/admin/tutor_detail/${row._id}`);
  };
  ...
  const filteredTeachers = tutor?.filter(
    (teacher) =>
  ...
  return (
    <div style={{ padding: "20px", height: "87vh", overflowY: "scroll" }}>
      ...
      >
        <h4 style={{ margin: "0" }}>Tutor Teacher List</h4>
        <div
          style={{
            display: "flex",
            alignItems: "center",
